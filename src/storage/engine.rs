...
pub trait Engine {
    ...
    fn scan_prefix(&mut self, prefix: Vec<u8>) -> Self::EngineIterator<'_> {
        ...
        // 前缀包含了 255
        // 10 2 3 255, 右边界就是 10 2 4
        // 如果全都是 255，右边界就是 Unbounded
        let end = match bound_prefix.iter_mut().rposition(|b| *b != 255) {
            Some(pos) => {
                bound_prefix[pos] += 1;
...
mod tests {
    ...
    fn test_point_opt(mut eng: impl Engine) -> RSDBResult<()> {
        ...
        eng.set(b"".to_vec(), vec![5, 6, 7, 8])?;
        assert_eq!(eng.get(b"".to_vec())?, Some(vec![5, 6, 7, 8]));
        Ok(())
    }
